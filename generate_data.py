...
def generate_questions(start_index, batch_size):
    ...
    with open("data/questions.jsonl", "a") as questions_file:
        ...
        llm = LLM(name="generate-toolformer-question")
...
def load_seed_dataset():
    return load_questions(path="seed_tasks.jsonl", key="question")
...
prompt = """a response to the question"""

class Response(Type):
    response: str = Context(prompt)
...
def load_questions_and_answers(path, question_key="question", answer_key="response"):
    questions_and_answers = []
    with open(path) as questions_file:
        reader = jsonlines.Reader(questions_file)

        for line in reader:
            if not answer_key in line:
                continue
            questions_and_answers.append([Question(
                question=line[question_key],
            ), Response(
                response=line[answer_key],
            )])
...
def generate_responses(index, batch_size):
    ...
    with open("data/dataset.jsonl", "a") as dataset_file:
        ...
        llm = LLM(name="generate-toolformer-reponse")
